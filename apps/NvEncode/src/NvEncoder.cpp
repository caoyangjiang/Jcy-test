                           ...
                           int dstStride)
{
  ...
  for (y = 0; y < height / 2; y++)
  {
    ...
    {
      nv12_chroma[(y * dstStride) + x] =
          yuv_cb[((srcStride / 2) * y) + (x >> 1)];
...
void convertYUV10pitchtoP010PL(unsigned short *yuv_luma,
                               unsigned short *yuv_cb,
                               unsigned short *yuv_cr,
                               unsigned short *nv12_luma,
                               unsigned short *nv12_chroma,
                               int width,
                               int height,
                               int srcStride,
                               int dstStride)
{
  ...
  for (y = 0; y < height / 2; y++)
  {
    for (x = 0; x < width; x = x + 2)
    {
      nv12_chroma[(y * dstStride / 2) + x] =
          yuv_cb[((srcStride / 2) * y) + (x >> 1)] << 6;
      nv12_chroma[(y * dstStride / 2) + (x + 1)] =
          yuv_cr[((srcStride / 2) * y) + (x >> 1)] << 6;
    }
  }
}
...
void convertYUV10pitchtoYUV444(unsigned short *yuv_luma,
                               unsigned short *yuv_cb,
                               unsigned short *yuv_cr,
                               unsigned short *surf_luma,
                               unsigned short *surf_cb,
                               unsigned short *surf_cr,
                               int width,
                               int height,
                               int srcStride,
                               int dstStride)
...
NVENCSTATUS CNvEncoder::InitD3D9(uint32_t deviceID)
{
  ...
  {
    PRINTERR(
        "Invalid Device Id = %d\n. Please use DX10/DX11 to detect headless "
        "video devices.\n",
        deviceID);
    return NV_ENC_ERR_INVALID_ENCODERDEVICE;
  }
  ...
  if (FAILED(hr)) return NV_ENC_ERR_OUT_OF_MEMORY;
...
NVENCSTATUS CNvEncoder::AllocateMVIOBuffers(uint32_t uInputWidth,
                                            uint32_t uInputHeight,
                                            NV_ENC_BUFFER_FORMAT inputFormat)
{
  NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
  ...
  for (uint32_t i = 0; i < m_uEncodeBufferCount; i++)
  {
    ...
    for (uint32_t j = 0; j < 2; j++)
    {
      nvStatus = m_pNvHWEncoder->NvEncCreateInputBuffer(
          uInputWidth,
          uInputHeight,
          &m_stMVBuffer[i].stInputBfr[j].hInputSurface,
          inputFormat);
      if (nvStatus != NV_ENC_SUCCESS) return nvStatus;
    ...
    nvStatus                   = m_pNvHWEncoder->NvEncCreateMVBuffer(
        dwSize, &m_stMVBuffer[i].stOutputBfr.hBitstreamBuffer);
    ...
    {
      nvStatus = m_pNvHWEncoder->NvEncRegisterAsyncEvent(
          &m_stMVBuffer[i].stOutputBfr.hOutputEvent);
      if (nvStatus != NV_ENC_SUCCESS) return nvStatus;
...
NVENCSTATUS CNvEncoder::ReleaseMVIOBuffers()
{
  for (uint32_t i = 0; i < m_uEncodeBufferCount; i++)
  {
    ...
    m_pNvHWEncoder->NvEncDestroyMVBuffer(
        m_stMVBuffer[i].stOutputBfr.hBitstreamBuffer);
...
NVENCSTATUS CNvEncoder::Deinitialize(uint32_t devicetype)
{
  ...
  if (m_pDevice)
  {
    ...
    {
#if defined(NV_WINDOWS)
      case NV_ENC_DX9: ((IDirect3DDevice9 *)(m_pDevice))->Release(); break;

      case NV_ENC_DX10: ((ID3D10Device *)(m_pDevice))->Release(); break;

      case NV_ENC_DX11: ((ID3D11Device *)(m_pDevice))->Release(); break;
#endif
...
void PrintHelp()
{
  printf(
      ...
      "-startf <integer>            Specify start index for encoding. Default "
      "is 0\n"
      "-endf <integer>              Specify end index for encoding. Default is "
      "end of file\n"
      ...
      "-i_qfactor <float>           Specify qscale difference between I-frames "
      "and P-frames\n"
      "-b_qfactor <float>           Specify qscale difference between P-frames "
      "and B-frames\n"
      "-i_qoffset <float>           Specify qscale offset between I-frames and "
      "P-frames\n"
      "-b_qoffset <float>           Specify qscale offset between P-frames and "
      "B-frames\n"
      ...
      "                                 2: YUV 420 10-bit\n"
      "                                 3: YUV 444 10-bit\n"
      "-deviceID <integer>           Specify the GPU device on which encoding "
      "will take place\n"
      "-meonly <integer>             Specify Motion estimation only(permissive "
      "value 1 and 2) to generates motion vectors and Mode information\n"
      "                                 1: Motion estimation between startf "
      "and endf\n"
      "                                 2: Motion estimation for all "
      "consecutive frames from startf to endf\n"
...
int CNvEncoder::EncodeMain(int argc, char *argv[])
{
  ...
  if (!encodeConfig.inputFileName || !encodeConfig.outputFileName ||
      encodeConfig.width == 0 || encodeConfig.height == 0)
  {
    PrintHelp();
    return 1;
  }
  ...
  if ((encodeConfig.enableMEOnly == 1) || (encodeConfig.enableMEOnly == 2))
  {
    ...
    {
      PRINTERR(
          "\nvEncoder.exe Error: MEOnly mode is now only supported for H264 "
          "and HEVC. Check input params!\n");
      return 1;
    }
  ...
  //   {
  // #if defined(NV_WINDOWS)
  //     case NV_ENC_DX9: InitD3D9(encodeConfig.deviceID); break;

  //     case NV_ENC_DX10: InitD3D10(encodeConfig.deviceID); break;

  //     case NV_ENC_DX11: InitD3D11(encodeConfig.deviceID); break;
  // #endif
  //     case NV_ENC_CUDA: InitCuda(encodeConfig.deviceID); break;
  //   }

  // if (encodeConfig.deviceType != NV_ENC_CUDA)
  //   nvStatus =
  //       m_pNvHWEncoder->Initialize(m_pDevice, NV_ENC_DEVICE_TYPE_DIRECTX);
  // else
  nvStatus = m_pNvHWEncoder->Initialize(m_pDevice, NV_ENC_DEVICE_TYPE_CUDA);
  if (nvStatus != NV_ENC_SUCCESS) return 1;

  encodeConfig.presetGUID = m_pNvHWEncoder->GetPresetGUID(
      encodeConfig.encoderPreset, encodeConfig.codec);
  ...
  //        encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_NV12
  //            ? "YUV 420"
  //            : (encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV444
  //                   ? "YUV 444"
  //                   : (encodeConfig.inputFormat ==
  ...
  //            ? "DX9"
  //            : encodeConfig.deviceType == NV_ENC_DX10
  //                  ? "DX10"
  //                  : encodeConfig.deviceType == NV_ENC_DX11
  //                        ? "DX11"
  //                        : encodeConfig.deviceType == NV_ENC_CUDA ? "CUDA"
  //                                                                 :
  //                                                                 "INVALID");
  ...
  if (nvStatus != NV_ENC_SUCCESS) return 1;
  encodeConfig.maxWidth =
      encodeConfig.maxWidth ? encodeConfig.maxWidth : encodeConfig.width;
  encodeConfig.maxHeight =
      encodeConfig.maxHeight ? encodeConfig.maxHeight : encodeConfig.height;
  ...
  if (encodeConfig.numB > 0)
  {
    m_uEncodeBufferCount =
        encodeConfig.numB + 4;  // min buffers is numb + 1 + 3 pipelining
  }
  else
  {
    int numMBs = ((encodeConfig.maxHeight + 15) >> 4) *
                 ((encodeConfig.maxWidth + 15) >> 4);
    int NumIOBuffers;
  ...
  if (m_stEncoderInput.enableMEOnly)
  {
    // Struct MotionEstimationBuffer has capacity to store two inputBuffer in
    // single object.
    m_uEncodeBufferCount = m_uEncodeBufferCount / 2;
    nvStatus             = AllocateMVIOBuffers(
        encodeConfig.width, encodeConfig.height, encodeConfig.inputFormat);
  }
  else
  {
    nvStatus = AllocateIOBuffers(
        encodeConfig.width, encodeConfig.height, encodeConfig.inputFormat);
  }
  if (nvStatus != NV_ENC_SUCCESS) return 1;
  ...
  uint32_t chromaFormatIDC =
      encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV444 ||
              encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT
          ? 3
          : 1;
  lumaPlaneSize =
      encodeConfig.maxWidth * encodeConfig.maxHeight *
      (encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV420_10BIT ||
               encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT
           ? 2
           : 1);
  chromaPlaneSize =
      (chromaFormatIDC == 3) ? lumaPlaneSize : (lumaPlaneSize >> 2);
  nvGetFileSize(hInput, &fileSize);
  int totalFrames =
      fileSize / (lumaPlaneSize + chromaPlaneSize + chromaPlaneSize);
  ...
  {
    PRINTERR(
        "nvEncoder.exe Warning: -endf %d exceeds total video frame %d, using "
        "%d instead\n",
        encodeConfig.endFrameIdx,
        totalFrames,
        totalFrames);
    encodeConfig.endFrameIdx = totalFrames - 1;
  }
  ...
  if ((encodeConfig.enableMEOnly == 1) || (encodeConfig.enableMEOnly == 2))
  {
    ...
    stMEOnly.stride[1] = ((chromaFormatIDC == 3) ? encodeConfig.width
                                                 : encodeConfig.width >> 1) *
                         nBytesPerSample;
    stMEOnly.stride[2] = ((chromaFormatIDC == 3) ? encodeConfig.width
                                                 : encodeConfig.width >> 1) *
                         nBytesPerSample;
    ...
    if (encodeConfig.enableMEOnly == 1)
    {
      ...
      for (unsigned int i = 0; i < 2; i++)
      {
        ...
        if (stMEOnly.yuv[i][0] == NULL || stMEOnly.yuv[i][1] == NULL ||
            stMEOnly.yuv[i][2] == NULL)
        {
          PRINTERR(
              "\nvEncoder.exe Error: Failed to allocate memory for array "
              "yuvLoaded of Size = %u !\n",
      ...
      numBytesRead = 0;
      loadframe(stMEOnly.yuv[0],
                hInput,
                encodeConfig.startFrameIdx,
                encodeConfig.width,
                encodeConfig.height,
                numBytesRead,
                encodeConfig.inputFormat);
      loadframe(stMEOnly.yuv[1],
                hInput,
                encodeConfig.endFrameIdx,
                encodeConfig.width,
                encodeConfig.height,
    ...
    else
    {
      ...
      unsigned int numFramesToEncode =
          MAX(1, (encodeConfig.endFrameIdx - encodeConfig.startFrameIdx));
      yuvLoaded[0] =
          new (std::nothrow) unsigned char[preloadedFrameCount * lumaPlaneSize];
      yuvLoaded[1] = new (
          std::nothrow) unsigned char[preloadedFrameCount * chromaPlaneSize];
      yuvLoaded[2] = new (
          std::nothrow) unsigned char[preloadedFrameCount * chromaPlaneSize];
      ...
      for (unsigned int iNumFrms = encodeConfig.startFrameIdx;
           iNumFrms < (unsigned int)encodeConfig.endFrameIdx + 1;
           iNumFrms += (preloadedFrameCount - 1))
      {
        if (iNumFrms != (MIN(iNumFrms + preloadedFrameCount - 1,
                             (unsigned int)encodeConfig.endFrameIdx)))
          printf("\nLoading Frames [%d,%d] into system memory\n",
                 iNumFrms,
                 (MIN(iNumFrms + preloadedFrameCount - 1,
                      (unsigned int)encodeConfig.endFrameIdx)));

        for (unsigned int frameCount = iNumFrms;
             frameCount < MIN(iNumFrms + preloadedFrameCount,
                              (unsigned int)encodeConfig.endFrameIdx + 1);
             frameCount++)
        {
          yuvInput[0] =
              &yuvLoaded[0][(frameCount % preloadedFrameCount) * lumaPlaneSize];
          yuvInput[1] = &yuvLoaded[1][(frameCount % preloadedFrameCount) *
                                      chromaPlaneSize];
          yuvInput[2] = &yuvLoaded[2][(frameCount % preloadedFrameCount) *
                                      chromaPlaneSize];
          loadframe(yuvInput,
                    hInput,
                    frameCount,
        ...
        for (unsigned int frameCount = iNumFrms;
             frameCount < MIN(iNumFrms + preloadedFrameCount - 1,
                              (unsigned int)encodeConfig.endFrameIdx);
             frameCount++)
        {
          numBytesRead = 0;
          ...
          stMEOnly.yuv[0][1] =
              &yuvLoaded[1]
                        [(frameCount % preloadedFrameCount) * chromaPlaneSize];
          stMEOnly.yuv[0][2] =
              &yuvLoaded[2]
                        [(frameCount % preloadedFrameCount) * chromaPlaneSize];
          // input
          stMEOnly.yuv[1][0] =
              &yuvLoaded[0][((frameCount + 1) % preloadedFrameCount) *
                            lumaPlaneSize];
          stMEOnly.yuv[1][1] =
              &yuvLoaded[1][((frameCount + 1) % preloadedFrameCount) *
                            chromaPlaneSize];
          stMEOnly.yuv[1][2] =
              &yuvLoaded[2][((frameCount + 1) % preloadedFrameCount) *
                            chromaPlaneSize];
          RunMotionEstimationOnly(&stMEOnly, false);
  ...
  for (int frm = encodeConfig.startFrameIdx; frm <= encodeConfig.endFrameIdx;
       frm++)
  {
    numBytesRead = 0;
    loadframe(yuv,
    ...
    stEncodeFrame.stride[0] =
        encodeConfig.width *
        (encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV420_10BIT ||
                 encodeConfig.inputFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT
             ? 2
             : 1);
    stEncodeFrame.stride[1] = stEncodeFrame.stride[2] =
        chromaFormatIDC == 3 ? stEncodeFrame.stride[0]
                             : stEncodeFrame.stride[0] / 2;
...
NVENCSTATUS CNvEncoder::RunMotionEstimationOnly(MEOnlyConfig *pMEOnly,
                                                bool bFlush)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    nvStatus                       = m_pNvHWEncoder->NvEncLockInputBuffer(
        pMEBuffer->stInputBfr[i].hInputSurface,
        (void **)&pInputSurface,
        &lockedPitch);
    if (nvStatus != NV_ENC_SUCCESS) return nvStatus;
    ...
    {
      pInputSurfaceCh =
          pInputSurface + (pMEBuffer->stInputBfr[i].dwHeight * lockedPitch);
      convertYUVpitchtoNV12(pMEOnly->yuv[i][0],
                            pMEOnly->yuv[i][1],
                            pMEOnly->yuv[i][2],
                            pInputSurface,
                            pInputSurfaceCh,
    ...
    else if (pMEBuffer->stInputBfr[i].bufferFmt == NV_ENC_BUFFER_FORMAT_YUV444)
    {
      ...
      unsigned char *pInputSurfaceCr =
          pInputSurfaceCb + (pMEBuffer->stInputBfr[i].dwHeight * lockedPitch);
      convertYUVpitchtoYUV444(pMEOnly->yuv[i][0],
                              pMEOnly->yuv[i][1],
                              pMEOnly->yuv[i][2],
                              pInputSurface,
                              pInputSurfaceCb,
    ...
    else if (pMEBuffer->stInputBfr[i].bufferFmt ==
             NV_ENC_BUFFER_FORMAT_YUV420_10BIT)
    {
      unsigned char *pInputSurfaceCh =
          pInputSurface + (pMEBuffer->stInputBfr[i].dwHeight * lockedPitch);
      convertYUV10pitchtoP010PL((uint16_t *)pMEOnly->yuv[i][0],
                                (uint16_t *)pMEOnly->yuv[i][1],
                                (uint16_t *)pMEOnly->yuv[i][2],
                                (uint16_t *)pInputSurface,
                                (uint16_t *)pInputSurfaceCh,
                                dwCurWidth,
                                dwCurHeight,
                                dwCurWidth,
                                lockedPitch);
    ...
    else
    {
      ...
      unsigned char *pInputSurfaceCr =
          pInputSurfaceCb + (pMEBuffer->stInputBfr[i].dwHeight * lockedPitch);
      convertYUV10pitchtoYUV444((uint16_t *)pMEOnly->yuv[i][0],
                                (uint16_t *)pMEOnly->yuv[i][1],
                                (uint16_t *)pMEOnly->yuv[i][2],
                                (uint16_t *)pInputSurface,
                                (uint16_t *)pInputSurfaceCb,
                                (uint16_t *)pInputSurfaceCr,
                                dwCurWidth,
                                dwCurHeight,
                                dwCurWidth,
                                lockedPitch);
                                    ...
                                    uint32_t height)
{
  ...
  {
    unsigned char *pInputSurfaceCh =
        pInputSurface + (pEncodeBuffer->stInputBfr.dwHeight * lockedPitch);
  ...
  else if (pEncodeBuffer->stInputBfr.bufferFmt == NV_ENC_BUFFER_FORMAT_YUV444)
  {
    unsigned char *pInputSurfaceCb =
        pInputSurface + (pEncodeBuffer->stInputBfr.dwHeight * lockedPitch);
    unsigned char *pInputSurfaceCr =
        pInputSurfaceCb + (pEncodeBuffer->stInputBfr.dwHeight * lockedPitch);
  ...
  else if (pEncodeBuffer->stInputBfr.bufferFmt ==
           NV_ENC_BUFFER_FORMAT_YUV420_10BIT)
  {
    unsigned char *pInputSurfaceCh =
        pInputSurface + (pEncodeBuffer->stInputBfr.dwHeight * lockedPitch);
    convertYUV10pitchtoP010PL((uint16_t *)pEncodeFrame->yuv[0],
                              (uint16_t *)pEncodeFrame->yuv[1],
                              (uint16_t *)pEncodeFrame->yuv[2],
                              (uint16_t *)pInputSurface,
                              (uint16_t *)pInputSurfaceCh,
                              width,
                              height,
        ...
        // NV_ENC_BUFFER_FORMAT_YUV444_10BIT)
  {
    ...
    unsigned char *pInputSurfaceCr =
        pInputSurfaceCb + (pEncodeBuffer->stInputBfr.dwHeight * lockedPitch);
    convertYUV10pitchtoYUV444((uint16_t *)pEncodeFrame->yuv[0],
                              (uint16_t *)pEncodeFrame->yuv[1],
                              (uint16_t *)pEncodeFrame->yuv[2],
                              (uint16_t *)pInputSurface,
                              (uint16_t *)pInputSurfaceCb,
  ...
  nvStatus = m_pNvHWEncoder->NvEncEncodeFrame(
      pEncodeBuffer, NULL, width, height, (NV_ENC_PIC_STRUCT)m_uPicStruct);
  return nvStatus;
}
